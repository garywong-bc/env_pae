...
function moh_preprocess_html(&$vars) {
  ...
  // Add the addthis javascript reference to the head of the document
  // drupal_add_js('http://s7.addthis.com/js/250/addthis_widget.js#pubid=xa-4f5f713709e4ec63 type="text/javascript"', 'external');  
  // Removed and added locally in the moh.info file
   
  drupal_add_js('var addthis_config = { services_compact: "facebook, twitter, more", services_exclude: "print, email" }',
    array(
		'type' 		=> 'inline', 
		'scope' 	=> 'header', 
		'everypage' => TRUE,
	...
	if (user_is_logged_in() == FALSE) {
		if ($element['#href'] != 'user/logout' && $element['#title'] != 'My Account') {
			...
			if ($element['#title'] == 'Login'){
				$output = l("Login", "account", array('query' => drupal_get_destination()), $element['#localized_options']);
			}
			else {
