...
<div id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>>
  ...
  <div class="content"<?php print $content_attributes; ?>>
	...
	<div class="content"<?php print $content_attributes; ?>>
	<?php	
		// Display comments and voting widget if user is logged in, otherwise display link to login page
		if ($user->uid) {
			
			$date = date('Y-m-d H:i:s');
			$current_date = new DateTime($date);
			
			if ( isset($node->field_closing_date) ) {
				$closing = $node->field_closing_date['und'][0]['value'];
				$closing_date = new DateTime($closing);
				
				if ($current_date <= $closing_date) {
					print('<p><label>Select one of the following/or enter comment below:</label></p>');
					print render($content['rate_support_tally']);
					print render($content['comments']);
				}
				else {
					print('<strong>Comments are closed.</strong>');
				}
			}
			else {
				hide($content['rate_support_tally']);
			...
			print l("Log in to post comments","account",array('query' => drupal_get_destination()));
